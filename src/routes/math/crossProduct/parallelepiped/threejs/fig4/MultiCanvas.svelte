<script lang="ts">
  ...
  import * as mj from 'mathjs';
    ...
    const rowop = mj.add(mj.multiply(2,vecv), vecb) as number[];
		...
		multiverse.start();
		d3Fig4d= new Fig4d({x:vecv[0],y:vecv[1]},{x:vecb[0],y:vecb[1]}, {x:rowop[0],y:rowop[1]});
...
<p><u><b>Step 2 Proof:</b></u></p>
    ...
    <p>In Figure 4.c, we place the two parallelepipeds together and the yellow plane is formed by <Latex  math={'\\vec{v}'}/> and <Latex  math={'\\vec{b}'}/>,
	...
	you are only operating on the plane that is formed by this two vectors, so the result vector also resides on the same plane. </p>
<p>With <Latex  math={'\\vec{v}'}/>, <Latex math={'\\vec{b}'} />, and <Latex  math={`\\vec{b'}`}/> being coplanar. You can see that 
<!-- svelte-ignore a11y-invalid-attribute -->
despite the base shapes of the parallelepipeds are different. <a href='#' on:click={()=>{universe3.eventBroker.emit('toggleShowHeight')}}>They have the same height</a>.  </p>
<p>So if we can show that those two parallelepipeds have the same area, then we know they have the same volume, which then proves that 
	row operations does not change the volume of a matrix from by 3 different vectors.
</p>

<p>To show the two parallelepipeds have the same area, we just need to show that the area formed by <Latex  math={'\\vec{v}'}/> and <Latex math={'\\vec{b}'} /> is equal to 
	the area formed by <Latex  math={'\\vec{v}'}/> and <Latex math={`\\vec{b'}`} />
</p>
...
are the same, which implies that they have the same area.
	...
	<div style="width:500px; height:400px; position:relative; display:flex; flex-flow: column nowrap;" bind:this={fig4dD3} >Figure 4.d</div>
