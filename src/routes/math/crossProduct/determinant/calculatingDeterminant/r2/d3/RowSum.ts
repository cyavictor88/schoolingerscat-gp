...
export class RowSum {
  ...
  constructor(mult : number,
    ...
    this.makeSeg(vecbPrime, {x:veca.x*mult,y:veca.y*mult},'black');
    // this.makeLine(veca,'#b3b3cc');


    this.makeVec({x:veca.x*mult,y:veca.y*mult},mult+'a','#e066ff', mult+`\\overrightarrow{a}`);
    this.makeVec(vecbPrime,mult+'a+b','purple', `\\overrightarrow{b\\prime}`);
