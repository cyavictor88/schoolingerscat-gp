...
export class Universe implements IUniverse {
  ...
  vecb: Vector;
  vecv: Vector;
  vecb2: Vector | null = null;
  axes: Axes;
  constantC: number | null = null;
  v1Line: Line;
  ...
  constructor(refCurrent: HTMLDivElement, veca: [number, number, number], vecb: [number, number, number], vecc: [number, number, number], zoomIn: boolean, constantC:number|null) {
    this.zoomIn = zoomIn;
    ...
    if (!zoomIn) {
      this.camera.position.set(5.44, 22., 22);
    ...
    if(this.constantC){
      const vecb2nums = vecb.map(n=>n*this.constantC!) as [number,number,number];
      this.vecb2 = new Vector(...vecb2nums, 0x800080 );
    } 
    if (!zoomIn) {
    ...
    if(this.vecb2) this.scene.add(this.vecb2.vector);
    ...
    if(this.vecb2){
      const pp2 = new Parallelepiped(this.scene, this.veca.coord, this.vecb2.coord, this.vecv.coord,'purple');
    }
  ...
  async setRightHand() {
    ...
    let det = mj.det([this.veca.coord.toArray(),this.vecb.coord.toArray(),this.vecv.coord.toArray()]);
    if(this.vecb2){
     det = mj.det([this.veca.coord.toArray(),this.vecb2.coord.toArray(),this.vecv.coord.toArray()]);
    }
    ...
    if(this.vecb2) vecb.copy(this.vecb2.coord);
    else vecb.copy(this.vecb.coord);
  ...
  async setMathMeshes() {
    ...
    if(this.vecb2){
      const mathTextb2 = await MathText.Init(`\\vec{${this.constantC}v_2}=(${this.vecb2.coord.x / scale},${this.vecb2.coord.y / scale},${this.vecb2.coord.z / scale})`, 'purple');
      mathTextb2.mesh.position.set(this.vecb2.coord.x, this.vecb2.coord.y, this.vecb2.coord.z)
      this.scene.add(mathTextb2.mesh);
    }
