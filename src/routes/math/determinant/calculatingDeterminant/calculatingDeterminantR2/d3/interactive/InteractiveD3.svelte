...
<div style="display:flex; flex-flow: row nowrap; gap:10px;">
	{#if d3Obj}
      ...
      {#if !swapRow}
			<p style="width:200px">
				<Latex
					math={`\\color{red} \\vec{a}=[${v1.x.toFixed(2)},${v1.y.toFixed(2)}]^{\\mathsf{T}}`}
				/>
			</p>
			<p>
				<Latex
					math={`\\color{blue} \\vec{b}=[${v2.x.toFixed(2)},${v2.y.toFixed(2)}]^{\\mathsf{T}}`}
				/>
			</p>
      {:else}
			<p>
				<Latex
					math={`\\color{blue} \\vec{b}=[${v1.x.toFixed(2)},${v1.y.toFixed(2)}]^{\\mathsf{T}}`}
				/>
			</p>
      <p style="width:200px">
				<Latex
					math={`\\color{red} \\vec{a}=[${v2.x.toFixed(2)},${v2.y.toFixed(2)}]^{\\mathsf{T}}`}
				/>
			</p>
      {/if}
			<p>
