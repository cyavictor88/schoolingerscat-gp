...
export async function mathmesh(input:string):Promise<ThreeD>{
    ...
    const mesh3D: Geometry  = await mmp.generateMathmesh3D();
    return mesh3D as ThreeD;
};

export async function mathmesh2D(input:string): Promise<TwoD> { 
    ...
    const mesh3D = await mathmesh(input);
    return get2D(mesh3D);
}

export async function mathmeshBoxAtLevel(input:string,lvl:number):Promise<{ positions: number[]; indices: number[]; vertices: Float32Array; }> {
   ...
   // console.log(html);
    ...
    let verts:{positions:number[],indices:number[],vertices:Float32Array}  = await mmp.generateBoundingBoxAtLevel(lvl);
    return verts;
